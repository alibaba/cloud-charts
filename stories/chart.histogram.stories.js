...
const options = {
  ...
  legend: {
    nameFormatter: value =>
      `${value}: ${data.find(x => x.name === value).data.length}`,
  },
};
...
const options_2 =  {
  tooltip: {
    titleFormatter: value => {
      ...
      return `${start}-${stop - 1}分`;
    },
    nameFormatter: () => '人数',
  },
  legend: {
    nameFormatter: name => `${name}（总人数：${data[0].data.length}）`,
  },
  guide: {
    line: {
      top: false,
...
stories.add('直方图', () => (
  <Wcontainer className="demos">
    <Whistogram height="300" config={options} data={data} />
  </Wcontainer>
));

stories.add('添加辅助线', () => (
  <Wcontainer className="demos">
    <Whistogram height="300" config={options_2} data={data_2} />
  </Wcontainer>
));

stories.add('极坐标', () => (
  <Wcontainer className="demos">
    <Whistogram height="300" config={{ ...options, polar: true }} data={data} />
  </Wcontainer>
));
