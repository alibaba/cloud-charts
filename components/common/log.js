...
setTimeout(() => {
  if (trackable && process.env.NODE_ENV === 'production') {
    ...
    image.src = `${logUrl}?version=${__VERSION__}&theme=${currentTheme || __THEME__}&t=${Date.now()}&host=${location && location.host}&fullurl=${document.URL}&chartinit=${chartInit}&uamobile=${isMobile}`;
  }
}, 3000);
