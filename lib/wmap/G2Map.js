...
var defaultConfig = {
  ...
  background: {
    ...
    stroke: _normal.color.widgetsMapAreaBorder
  },
  areaColors: _normal.color.order_10,
...
function drawMapBackground(chart, ds, config) {
  ...
  bgMapView.polygon().position('x*y').style('name', _extends({
    fill: bgFill || _normal.color.widgetsMapAreaBg,
    stroke: function stroke(name) {
      // 对一些尺寸非常小的形状特殊处理，以显示出来。
      if (minArea.indexOf(name) > -1) {
        return bgFill || _normal.color.widgetsMapAreaBg;
      }
      return bgStroke || _normal.color.widgetsMapAreaBorder;
    },
    lineWidth: 1
  }, otherBgStyle));
  ...
  if (pointMapDataView) {
    ...
    if (config.labels) {
      ...
      pointGeom.label('name', {
        offset: '' + (offset - Number(_normal.size.s3.replace('px', ''))),
        textStyle: _extends({
