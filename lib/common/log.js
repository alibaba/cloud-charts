...
setTimeout(function () {
  if (trackable && process.env.NODE_ENV === 'production') {
    ...
    image.src = logUrl + "?version=" + _constants.__VERSION__ + "&theme=" + (currentTheme || _constants.__THEME__) + "&t=" + Date.now() + "&host=" + (location && location.host) + "&chartinit=" + chartInit + "&uamobile=" + _platform.isMobile;
  }
}, 3000);