 ...
 * */
function _default(chart, config, defs, transpose) {
  // X轴&Y轴对应的长度
  var xLen = transpose ? chart.get('height') : chart.get('width');
  var yLen = transpose ? chart.get('width') : chart.get('height'); // 坐标系不翻转时为横向，翻转时为纵向

  computerAxis(chart, config.xAxis, defs.x, transpose, xLen); // 坐标系不翻转时为纵向，翻转时为横向
...
function computerAxis(chart, axisConfig, def, isVertical, len) {
  ...
  var labelFontSize = (0, _common.pxToNumber)(_themes["default"]['widgets-font-size-1']);
...
function convertFontSize(fontSize) {
  ...
  if ((0, _common.isInvalidNumber)(size)) {
    return (0, _common.pxToNumber)(_themes["default"]['widgets-font-size-1']);
  } else {
    return Number(size);
  }
}