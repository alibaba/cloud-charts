...
function getPadding(position, userPadding) {
  if (userPadding) {
    return userPadding;
  }

  var len = (0, _common.pxToNumber)(_themes["default"]['widgets-font-size-1']) * 2 / 3;
...
function _default(chart, config, defaultConfig, isOneDataGroup, field) {
  // 设置图例
  if (config.legend === false || config.legend && config.legend.visible === false) {
    ...
    var legendConfig = (0, _extends2["default"])({}, defaultConfig, {
      ...
      marker: marker || {
        // symbol: 'circle',
        style: {
          r: (0, _common.pxToNumber)(_themes["default"]['widgets-font-size-1']) / 3,
    ...
    if (customConfig) {
      (0, _common.merge)(legendConfig, customConfig);
