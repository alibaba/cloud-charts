...
function g2Factory(name, Chart) {
  ...
  var AiscChart = function (_React$Component) {
    ...
    AiscChart.prototype.checkConfigChange = function checkConfigChange(newConfig, oldConfig) {
    ...
    AiscChart.prototype.initChart = function initChart(props) {
      ...
      if (config.xAxis && config.xAxis.type === 'datetime') {
        console.warn('配置属性 "config.xAxis.type": "datetime" 在 widgets 2.x 中已被废弃，请使用 "config.xAxis.type": "time"');
      }
