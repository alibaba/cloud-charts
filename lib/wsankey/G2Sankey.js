...
exports.default = {
  ...
  init: function init(chart, userConfig, data) {
    ...
    edgeView.edge().position('x*y').shape('arc').color(_normal.color.widgetsSankeyEdge).opacity(0.5).tooltip('target*source*value', config.tooltip.nameFormatter);
    ...
    if (config.labels) {
      nodeGeom.label('name', {
        textStyle: {
          fill: _normal.color.widgetsSankeyNodeText,
          textAlign: 'start'
        },
        offset: 0,
