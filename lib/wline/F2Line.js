...
var guideColorMap = [color.widgetsColorRed, color.widgetsColorOrange];
...
var renderTooltip = function renderTooltip(elem, x, dataArr, canvas, config) {
  ...
  if (!topContentDiv) {
    ...
    var parent = document.getElementById(getContainerId(elem.chartId));
...
var line = {
  ...
  clearToolTip: function clearToolTip(elem) {
    var parentContainer = document.querySelector('#' + getContainerId(elem.chartId));
    var id = getTooltipId(elem.chartId);
    // 如果有辅助线，则清除
  ...
  afterRender: function afterRender(canvas, chart, config, props, elem, self) {
    ...
    var dirTop = config.legend.position === 'top';
    var point = getPoint(canvas, chart.get('width'), 0);
    var data = chart.getSnapRecords(point);
    var dataArr = [];
    var resultArr = [];
    data.forEach(function (i) {
