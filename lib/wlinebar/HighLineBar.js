...
var LineBar = function (_Base) {
  _inherits(LineBar, _Base);

  function LineBar(selector, options) {
    _classCallCheck(this, LineBar);

    var _this = _possibleConstructorReturn(this, (LineBar.__proto__ || Object.getPrototypeOf(LineBar)).call(this, selector, options));
  ...
  _createClass(LineBar, [{
    ...
    value: function init() {
      ...
      this.element.classList.add('p2c-line-bar');
    ...
    value: function destroy() {
      _get(LineBar.prototype.__proto__ || Object.getPrototypeOf(LineBar.prototype), 'destroy', this).call(this);
    ...
    value: function render() {
      ...
      var legendNode = !!this.options.legend && this.element.querySelector('.p2c-legend');
      ...
      if (this.data.length) {
        ...
        options.xAxis.events.setExtremes = function (event) {
          ...
          } else if (!self.chart.resetZoomButton) self.chart.showResetZoom();else self.chart.resetZoomButton.show();
        };
        ...
        if (!this.chart) {
          this.data.forEach(function (item, index) {
            options.series.push({
              type: item.type === 'bar' ? 'column' : lineType,
              data: item.data,
          ...
          this.data.forEach(function (item, index) {
            if (_this2.chart.series[index]) {
              ...
              _this2.chart.series[index].update({
                type: item.type === 'bar' ? 'column' : lineType,
                color: _this2.options.colors[index],
                lineColor: _this2.options.colors[index],
                name: item.name,
          ...
          if (Array.isArray(options.yAxis)) {
            options.yAxis.forEach(function (y, index) {
              if (_this2.chart.yAxis[index]) {
                ...
                _this2.chart.addAxis(getYAxis(_this2.options, y, index));
              }
            });
        ...
        if (legendNode) {
          ...
          Array.prototype.forEach.call(legendNode.querySelectorAll('li'), function (item) {
            item.addEventListener('click', function (e) {
              ...
              } else if (_this2.chart.series[id]) _this2.chart.series[id].setVisible(true, false);
    ...
    value: function visibleHandler(e) {
      ...
      series.forEach(function (s, i) {
        ...
        } else if (_this4.chart.series[i]) _this4.chart.series[i].setVisible(false, false);
      });
      this.chart.redraw(false);
    }
  }]);
...
function getLegend(node, options, data) {
  ...
  data.forEach(function (item, i) {
    ...
    if (options.legend.nameFormatter) {
      name = options.legend.nameFormatter(name, _extends({}, item, { color: options.colors[i] }), i);
    }
...
function getHCOptions(options, data) {
  ...
  var config = {
    ...
    tooltip: {
      ...
      useHTML: true,
      backgroundColor: _variables.colors.colorTransparent,
      borderColor: _variables.colors.colorTransparent,
      shadow: false
    },
    legend: false,
    xAxis: {
      ...
      gridLineColor: _variables.colors.colorFill12,
      tickPixelInterval: 70,
      lineColor: _variables.colors.colorLine12,
      tickLength: 0,
      labels: {
        autoRotation: false,
        y: 20,
        ...
        style: { fontFamily: _variables.fonts.fontFamilyBase, fontSize: _variables.fonts.fontSizeBaseCaption, color: '#989898' }
      },
      minPadding: 0,
    ...
    plotOptions: {
        ...
        marker: {
          ...
          states: {
            hover: {
              lineWidthPlus: 0,
              radiusPlus: 0,
              fillColor: _variables.colors.colorWhite,
              lineWidth: 2,
              radius: 3.5,
      ...
      series: {
        ...
        dataLabels: {
          ...
          shadow: false,
          // inside: false,
          style: { fontFamily: _variables.fonts.fontFamilyBase, fontWeight: _variables.fonts.fontWeightRegular, fontSize: _variables.fonts.fontSizeBaseCaption, color: '#989898' },
          formatter: function formatter() {
...
function getYAxis(options, data, yAxis, index) {
  ...
  return {
    ...
    lineColor: index === undefined ? _variables.colors.colorLine12 : getYAxisColor(options, data, index),
...
function getYAxisColor(options, data, index) {
  ...
  return _variables.colors.colorLine12;
}
...
function plotLinesFormat(plotLines) {
  if (plotLines && Array.isArray(plotLines)) {
    return plotLines.map(function (item) {
      return {
        color: item.color || _colors2.default.colorF41,
        dashStyle: 'dash',
        value: item.value,
        width: item.width || 1
      };
