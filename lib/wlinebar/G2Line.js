...
exports.default = {
  ...
  init: function init(chart, userConfig, data) {
    ...
    if (config.xAxis.type === 'datetime') {
      defs = (0, _merge2.default)({}, defs, !config.xAxis.labelFormatter ? {
        name: {
    ...
    var valueAxis = {
      ...
      grid: {
        line: {
          stroke: _variables.colors.colorLine12,
          lineWidth: 1,
          lineDash: [4, 0]
        }
      },
    ...
    if (config.legend) {
      ...
      for (var i = 0, l = items.length; i < l; i++) {
        ...
        var typeFormatter = config.legend.labelFormatter ? config.legend.labelFormatter(type) : type;
