...
exports.default = {
  ...
  init: function init(chart, userConfig, data) {
    ...
    var rawLineData = this.rawLineData = [];
    var rawBarData = this.rawBarData = [];
    ...
    if (config.legend !== false) {
      ...
      if (position === 'top') {
        legendStyle.top = _index.size.s3;
      }

      if (align === 'right') {
        legendStyle.marginLeft = _index.size.s3;
      } else if (align === 'left') {
        legendStyle.marginRight = _index.size.s3;
...
function drawBar(chart, config) {
  ...
  (0, _label2.default)(intervalGeom, config, yAxisKey);
}
